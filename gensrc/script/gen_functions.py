...
def is_german_string_function(entry):
    """True if the entry is a TYPE_GERMAN_STRING variant (IDs reserved in 3800x).

    TYPE_GERMAN_STRING is an internal BE-only type used by the FE->BE Thrift
    rewrite pass. Its signatures are never resolved through FE's FunctionSet;
    the FE rewriter swaps VARCHAR fn_ids to the GERMAN_STRING fn_ids at plan
    Thrift-serialization time, using a static map emitted by this script.
    """
    return 38000 <= int(entry["id"]) < 39000
...
${default_values}
    ...
    # GERMAN_STRING overloads are never registered in FE's FunctionSet: the
    # FE rewrite pass swaps VARCHAR fn_ids to GS fn_ids via a static map
    # emitted by generate_gs_function_map(). See VectorizedGermanStringFunctionMap.
    fe_functions = [f for f in function_list if not is_german_string_function(f)]
    value["functions"] = "\n        ".join([gen_fe_fn(i) for i in fe_functions])
...
def generate_gs_function_map(path):
    """Emit a Java class mapping VARCHAR fn_id -> GS fn_id.

    For each GERMAN_STRING builtin (id in 3800x), find the VARCHAR counterpart
    by matching the function name and arg list (with GERMAN_STRING substituted
    by VARCHAR). The resulting static map is consumed by
    GermanStringRewriter to swap fn_ids at Thrift-serialization time.
    """
    varchar_by_sig = {}
    for entry in function_list:
        if is_german_string_function(entry):
            continue
        sig = (entry["name"], tuple(entry["args"]))
        # Prefer the first registration for a given (name, args).
        varchar_by_sig.setdefault(sig, entry["id"])

    mappings = []
    unmatched = []
    for entry in function_list:
        if not is_german_string_function(entry):
            continue
        varchar_args = tuple(
            "VARCHAR" if a == "GERMAN_STRING" else a for a in entry["args"]
        )
        sig = (entry["name"], varchar_args)
        varchar_id = varchar_by_sig.get(sig)
        if varchar_id is None:
            unmatched.append((entry["id"], entry["name"], entry["args"]))
            continue
        mappings.append((varchar_id, entry["id"], entry["name"]))

    if unmatched:
        print("=================================================================")
        print("GERMAN_STRING builtin without VARCHAR counterpart:")
        for uid, name, args in unmatched:
            print("    %d %s(%s)" % (uid, name, ", ".join(args)))
        print("=================================================================")
        exit(1)

    lines = []
    for varchar_id, gs_id, name in mappings:
        lines.append(
            "        MAP.put(%dL, %dL); // %s" % (varchar_id, gs_id, name)
        )
    body = "\n".join(lines)

    content = (
        license_string.lstrip()
        + """
package com.starrocks.builtins;
...
/**
 * VARCHAR -> GERMAN_STRING function id substitution table.
 *
 * <p>GERMAN_STRING is an internal BE-only type: it does not appear in SQL,
 * DDL, or the FE's FunctionSet. When the session variable
 * {@code enable_german_string} is on, the late-stage plan rewriter replaces
 * VARCHAR scalar function calls with their GERMAN_STRING counterparts just
 * before Thrift serialization, using this map.
 *
 * <p>This file is generated by {@code gensrc/script/gen_functions.py}; do not
 * edit it by hand.
 */
public final class VectorizedGermanStringFunctionMap {
...
"""
        + body.replace("        MAP.put", "        m.put")
        + """
        MAP = ImmutableMap.copyOf(m);
    }

    private VectorizedGermanStringFunctionMap() {
    }

    /**
     * Returns the GERMAN_STRING fn_id corresponding to the given VARCHAR
     * builtin fn_id, or {@code null} if no GS counterpart exists.
     */
    public static Long getGermanStringId(long varcharFnId) {
        return MAP.get(varcharFnId);
    }

    /**
     * Returns {@code true} iff {@code varcharFnId} has a GERMAN_STRING
     * counterpart registered in BE.
     */
    public static boolean hasGermanStringVariant(long varcharFnId) {
        return MAP.containsKey(varcharFnId);
    }
}
"""
    )
...
if __name__ == "__main__":
    ...
    generate_gs_function_map(
        fe_functions_dir + "/VectorizedGermanStringFunctionMap.java"
    )
