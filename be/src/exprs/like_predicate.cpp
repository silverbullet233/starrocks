...
Status LikePredicate::compile_with_hyperscan_or_re2(const std::string& pattern, LikePredicateState* state,
    ...
    if (!hs_compile_and_alloc_scratch(pattern, state, context, slice)) {
        ...
        if constexpr (full_match) {
            ...
            state->dispatch_kind = DispatchKind::LIKE_LONG_RE2;
        } else {
            state->function = &regex_fn_with_long_constant_pattern;
            state->dispatch_kind = DispatchKind::REGEX_LONG_RE2;
        }
    }
...
Status LikePredicate::like_prepare(FunctionContext* context, FunctionContext::FunctionStateScope scope) {
    ...
    state->dispatch_kind = DispatchKind::GENERAL_LIKE;
...
Status LikePredicate::regex_prepare(FunctionContext* context, FunctionContext::FunctionStateScope scope) {
    ...
    state->dispatch_kind = DispatchKind::GENERAL_REGEX;
...
// =============================================================================
// TYPE_GERMAN_STRING overloads. The prepare step is type-agnostic and reused
// verbatim; here we just mirror the per-row VARCHAR loops against
// `ColumnViewer<TYPE_GERMAN_STRING>`. Dispatch is driven by the
// `state->dispatch_kind` chosen in `like_prepare` / `regex_prepare`.
// =============================================================================

namespace {

// Compare `gs` against a fixed byte range `(ptr, size)`. When `size <=
// GermanString::PREFIX_LENGTH`, we can compare directly against the long-rep
// inline prefix without dereferencing `long_rep.ptr`. Inline strings already
// keep all bytes in `short_rep.str`; both representations share layout so the
// first 4 bytes after `len` are always available via `long_rep.prefix`.
inline bool gs_starts_with(const GermanString& gs, const char* ptr, uint32_t size) {
    if (size > gs.len) {
        return false;
    }
    if (size <= GermanString::PREFIX_LENGTH) {
        // long_rep.prefix overlays short_rep.str[0..3]; safe for both reps.
        return memcmp(gs.long_rep.prefix, ptr, size) == 0;
    }
    return memcmp(gs.get_data(), ptr, size) == 0;
}

inline bool gs_ends_with(const GermanString& gs, const char* ptr, uint32_t size) {
    if (size > gs.len) {
        return false;
    }
    return memcmp(gs.get_data() + gs.len - size, ptr, size) == 0;
}

inline bool gs_equals(const GermanString& gs, const char* ptr, uint32_t size) {
    if (size != gs.len) {
        return false;
    }
    return memcmp(gs.get_data(), ptr, size) == 0;
...
inline bool gs_contains(const GermanString& gs, const Slice& needle) {
    if (needle.size == 0) {
        return true;
    }
    if (needle.size > gs.len) {
        return false;
    }
    auto searcher = LibcASCIICaseSensitiveStringSearcher(needle.data, needle.size);
    return searcher.search(gs.get_data(), gs.len) != nullptr;
}

} // namespace

StatusOr<ColumnPtr> LikePredicate::constant_ends_with_fn_german_string(FunctionContext* context,
                                                                       const Columns& columns) {
    auto* state = reinterpret_cast<LikePredicateState*>(context->get_function_state(FunctionContext::THREAD_LOCAL));
    const auto& value_column = VECTORIZED_FN_ARGS(0);
    auto [all_const, num_rows] = ColumnHelper::num_packed_rows(columns);

    ColumnViewer<TYPE_GERMAN_STRING> value_viewer(value_column);
    ColumnBuilder<TYPE_BOOLEAN> result(num_rows);
    const Slice needle = state->search_string_sv;
    for (int row = 0; row < num_rows; ++row) {
        if (value_viewer.is_null(row)) {
            result.append_null();
            continue;
        }
        result.append(gs_ends_with(value_viewer.value(row), needle.data, needle.size));
    }
    return result.build(all_const);
}

StatusOr<ColumnPtr> LikePredicate::constant_starts_with_fn_german_string(FunctionContext* context,
                                                                         const Columns& columns) {
    auto* state = reinterpret_cast<LikePredicateState*>(context->get_function_state(FunctionContext::THREAD_LOCAL));
    const auto& value_column = VECTORIZED_FN_ARGS(0);
    auto [all_const, num_rows] = ColumnHelper::num_packed_rows(columns);
    ...
    for (int row = 0; row < num_rows; ++row) {
        ...
        result.append(gs_starts_with(value_viewer.value(row), needle.data, needle.size));
    }
    return result.build(all_const);
}

StatusOr<ColumnPtr> LikePredicate::constant_equals_fn_german_string(FunctionContext* context, const Columns& columns) {
    auto* state = reinterpret_cast<LikePredicateState*>(context->get_function_state(FunctionContext::THREAD_LOCAL));
    const auto& value_column = VECTORIZED_FN_ARGS(0);
    auto [all_const, num_rows] = ColumnHelper::num_packed_rows(columns);
    ...
    for (int row = 0; row < num_rows; ++row) {
        ...
        result.append(gs_equals(value_viewer.value(row), needle.data, needle.size));
    }
    return result.build(all_const);
}

StatusOr<ColumnPtr> LikePredicate::constant_substring_fn_german_string(FunctionContext* context,
                                                                       const Columns& columns) {
    RETURN_IF_COLUMNS_ONLY_NULL(columns);
    auto* state = reinterpret_cast<LikePredicateState*>(context->get_function_state(FunctionContext::THREAD_LOCAL));
    const auto& value_column = VECTORIZED_FN_ARGS(0);
    ...
    for (int row = 0; row < num_rows; ++row) {
        ...
        result.append(gs_contains(value_viewer.value(row), needle));
    }
    return result.build(all_const);
}

template <bool full_match>
StatusOr<ColumnPtr> LikePredicate::match_fn_with_long_constant_pattern_gs(FunctionContext* context,
                                                                          const Columns& columns) {
    auto* state = reinterpret_cast<LikePredicateState*>(context->get_function_state(FunctionContext::THREAD_LOCAL));
    ...
    for (int row = 0; row < num_rows; ++row) {
        ...
        const auto gs = value_viewer.value(row);
        bool v = false;
        if constexpr (full_match) {
            v = RE2::FullMatch(re2::StringPiece(gs.get_data(), gs.len), *(state->re2));
        } else {
            v = RE2::PartialMatch(re2::StringPiece(gs.get_data(), gs.len), *(state->re2));
        }
        result.append(v);
    }
    return result.build(all_const);
...
StatusOr<ColumnPtr> LikePredicate::like_fn_with_long_constant_pattern_german_string(FunctionContext* context,
                                                                                    const Columns& columns) {
    return match_fn_with_long_constant_pattern_gs<true>(context, columns);
}

StatusOr<ColumnPtr> LikePredicate::regex_fn_with_long_constant_pattern_german_string(FunctionContext* context,
                                                                                     const Columns& columns) {
    return match_fn_with_long_constant_pattern_gs<false>(context, columns);
}

// Hyperscan path shared by `like_fn_german_string` / `regex_fn_german_string`
// when the pattern is constant and a Hyperscan database is compiled.
StatusOr<ColumnPtr> LikePredicate::predicate_const_hs_gs(FunctionContext* context, ColumnBuilder<TYPE_BOOLEAN>* result,
    ...
    for (int row = 0; row < value_viewer.size(); ++row) {
        ...
        const auto gs = value_viewer.value(row);
        bool v = false;
        [[maybe_unused]] auto st = hs_scan(
                state->database, (gs.len) ? gs.get_data() : &_DUMMY_STRING_FOR_EMPTY_PATTERN, gs.len, 0, scratch,
                [](unsigned int id, unsigned long long from, unsigned long long to, unsigned int flags,
                   void* ctx) -> int {
                    *((bool*)ctx) = true;
                    return 1;
...
StatusOr<ColumnPtr> LikePredicate::regex_match_gs(FunctionContext* context, const Columns& columns) {
    RETURN_IF_COLUMNS_ONLY_NULL(columns);
    ...
    if (context->is_constant_column(1)) {
        if (!pattern_column->only_null()) {
            return predicate_const_hs_gs(context, &result, value_viewer, value_column);
        }
        return ColumnHelper::create_const_null_column(value_column->size());
    }

    // Non-constant pattern path: keep the pattern as VARCHAR (registered
    // signature is (GERMAN_STRING, VARCHAR)).
    ColumnViewer<TYPE_VARCHAR> pattern_viewer(pattern_column);
    ...
    for (int row = 0; row < num_rows; ++row) {
        ...
        const auto gs = value_viewer.value(row);
        const Slice pattern = pattern_viewer.value(row);
        ...
        if constexpr (is_like_pattern) {
            ...
            case FastPathType::EQUALS: {
                std::string str_pattern = pattern.to_string();
                remove_escape_character(&str_pattern);
                result.append(gs_equals(gs, str_pattern.data(), str_pattern.size()));
                break;
            }
            case FastPathType::START_WITH: {
                std::string str_pattern = pattern.to_string();
                remove_escape_character(&str_pattern);
                // trailing '%' stripped.
                result.append(gs_starts_with(gs, str_pattern.data(), str_pattern.size() - 1));
                break;
            }
            case FastPathType::END_WITH: {
                std::string str_pattern = pattern.to_string();
                remove_escape_character(&str_pattern);
                // leading '%' skipped.
                result.append(gs_ends_with(gs, str_pattern.data() + 1, str_pattern.size() - 1));
                break;
            }
            case FastPathType::SUBSTRING: {
                std::string str_pattern = pattern.to_string();
                remove_escape_character(&str_pattern);
                Slice pattern_slice = Slice(str_pattern);
                pattern_slice.remove_prefix(1);
                pattern_slice.remove_suffix(1);
                result.append(gs_contains(gs, pattern_slice));
                break;
            }
            case FastPathType::REGEX: {
                auto re_pattern = LikePredicate::template convert_like_pattern<false>(context, pattern);
                ...
                result.append(RE2::FullMatch(re2::StringPiece(gs.get_data(), gs.len), re));
                break;
            }
            }
        } else {
            auto re_pattern = pattern.to_string();
            re2::RE2 re(re_pattern, opts);
            if (!re.ok()) {
                context->set_error(strings::Substitute("Invalid regex: $0", re_pattern).c_str());
                result.append_null();
            ...
            result.append(RE2::PartialMatch(re2::StringPiece(gs.get_data(), gs.len), re));
        }
    }
    return result.build(all_const);
}

StatusOr<ColumnPtr> LikePredicate::like_fn_german_string(FunctionContext* context, const Columns& columns) {
    return regex_match_gs<true>(context, columns);
}

StatusOr<ColumnPtr> LikePredicate::regex_fn_german_string(FunctionContext* context, const Columns& columns) {
    return regex_match_gs<false>(context, columns);
}

// Top-level dispatcher: pick the correct GermanString row loop by inspecting
// the `dispatch_kind` bound by `like_prepare` / `regex_prepare`.
StatusOr<ColumnPtr> LikePredicate::like_german_string(FunctionContext* context, const Columns& columns) {
    auto* state = reinterpret_cast<LikePredicateState*>(context->get_function_state(FunctionContext::THREAD_LOCAL));
    switch (state->dispatch_kind) {
    case DispatchKind::CONST_ENDS_WITH:
        return constant_ends_with_fn_german_string(context, columns);
    case DispatchKind::CONST_STARTS_WITH:
        return constant_starts_with_fn_german_string(context, columns);
    case DispatchKind::CONST_EQUALS:
        return constant_equals_fn_german_string(context, columns);
    case DispatchKind::CONST_SUBSTRING:
        return constant_substring_fn_german_string(context, columns);
    case DispatchKind::LIKE_LONG_RE2:
        return like_fn_with_long_constant_pattern_german_string(context, columns);
    case DispatchKind::REGEX_LONG_RE2:
        return regex_fn_with_long_constant_pattern_german_string(context, columns);
    case DispatchKind::GENERAL_LIKE:
    case DispatchKind::GENERAL_REGEX:
    default:
        return like_fn_german_string(context, columns);
    }
}

StatusOr<ColumnPtr> LikePredicate::regex_german_string(FunctionContext* context, const Columns& columns) {
    auto* state = reinterpret_cast<LikePredicateState*>(context->get_function_state(FunctionContext::THREAD_LOCAL));
    switch (state->dispatch_kind) {
    case DispatchKind::CONST_ENDS_WITH:
        return constant_ends_with_fn_german_string(context, columns);
    ...
    case DispatchKind::GENERAL_REGEX:
    default:
        return regex_fn_german_string(context, columns);
    }
}
