...
std::pair<Columns, UInt32Column::Ptr> JsonEach::process(RuntimeState* runtime_state, TableFunctionState* state) const {
    ...
    Columns result;
    BinaryColumn::MutablePtr key_column_ptr = BinaryColumn::create(memory::get_default_allocator());
    JsonColumn::MutablePtr value_column_ptr = JsonColumn::create(memory::get_default_allocator());
    result.emplace_back(key_column_ptr);
    result.emplace_back(value_column_ptr);
    auto offset_column = UInt32Column::create(memory::get_default_allocator());
    int offset = 0;
