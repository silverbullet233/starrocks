...
struct SelectIfOP {
    static ColumnPtr eval(memory::Allocator* allocator, ColumnPtr& value0, ColumnPtr& value1, ColumnPtr& selector, const TypeDescriptor& type_desc) {
...
public:
    ...
    StatusOr<ColumnPtr> evaluate_checked(ExprContext* context, Chunk* ptr) override {
        ...
        if constexpr (lt_is_collection<Type>) {
            return _evaluate_complex(context, list);
        } else {
            return _evaluate_general(context, list);
        }
    }

private:
    ColumnPtr _evaluate_general(ExprContext* context, const Columns& columns) {
        ColumnViewer<Type> lhs_viewer(columns[0]);
        ColumnViewer<Type> rhs_viewer(columns[1]);
        auto [all_const, num_rows] = ColumnHelper::num_packed_rows(columns);

        ColumnBuilder<Type> result(context->get_allocator(), num_rows, this->type().precision, this->type().scale);
    ...
    ColumnPtr _evaluate_complex(ExprContext* context, const Columns& inputs) {
        auto num_rows = inputs[0]->size();
        Columns columns;
        for (const auto& col : inputs) {
            columns.push_back(ColumnHelper::unfold_const_column(context->get_allocator(), this->type(), num_rows, std::move(col)));
        }
        auto res = ColumnHelper::create_column(context->get_allocator(), this->type(), true);
...
private:
    ColumnPtr _evaluate_general(ExprContext* context, const Columns& columns) {
        ...
        ColumnBuilder<Type> result(context->get_allocator(), size, this->type().precision, this->type().scale);
...
public:
    ...
    StatusOr<ColumnPtr> evaluate_checked(ExprContext* context, Chunk* ptr) override {
        ...
        if (bhs_nulls == 0 && lhs_nulls == 0 && rhs_nulls == 0) {
            ...
            if constexpr (lt_is_collection<Type>) {
                return _evaluate_complex<false>(context->get_allocator(), list);
            } else if (bhs->is_constant() || !isArithmeticLT<Type>) {
                return _evaluate_general<false>(context, list);
            } else if constexpr (isArithmeticLT<Type>) {
                return dispatch_nonull_template<SelectIfOP, Type>(context->get_allocator(), lhs, rhs, bhs, type());
            } else {
                __builtin_unreachable();
            }
        } else {
            if constexpr (lt_is_collection<Type>) {
                return _evaluate_complex<true>(context->get_allocator(), list);
            } else if constexpr (isArithmeticLT<Type>) {
                // SIMD branch
                size_t num_rows = list[0]->size();
                // get null data
                auto lns = get_null_column(context->get_allocator(), num_rows, lhs);
                auto rns = get_null_column(context->get_allocator(), num_rows, rhs);
                // get data columns
                auto lds = get_data_column(context->get_allocator(), num_rows, lhs);
                auto rds = get_data_column(context->get_allocator(), num_rows, rhs);
                // call select if
                auto selector = bhs->only_null() ? UInt8Column::create(context->get_allocator(), num_rows) : bhs;
                auto select_data = dispatch_nonull_template<SelectIfOP, Type>(context->get_allocator(), lds, rds, bhs, type());
                auto select_null =
                        dispatch_nonull_template<SelectIfOP, TYPE_BOOLEAN>(context->get_allocator(), lns, rns, bhs, TypeDescriptor(TYPE_BOOLEAN));
...
private:
    ...
    ColumnPtr _evaluate_complex(memory::Allocator* allocator, const Columns& inputs) {
        ...
        Columns columns;
        for (const auto& col : inputs) {
            columns.push_back(ColumnHelper::unfold_const_column(allocator, this->type(), num_rows, std::move(col)));
        }
        ColumnViewer<TYPE_BOOLEAN> bhs_viewer(columns[0]);
...
public:
    ...
    StatusOr<ColumnPtr> evaluate_checked(ExprContext* context, Chunk* ptr) override {
        ...
        // return only null
        if (columns.size() == 0) {
            return ColumnHelper::create_const_null_column(context->get_allocator(), ptr != nullptr ? ptr->num_rows() : 1);
        }
        ...
        if constexpr (lt_is_collection<Type>) {
            return _evaluate_complex(context, columns);
        } else {
            return _evaluate_general(context, columns);
        }
    }

private:
    StatusOr<ColumnPtr> _evaluate_general(ExprContext* context, const Columns& columns) {
        ...
        ColumnBuilder<Type> builder(context->get_allocator(), size, this->type().precision, this->type().scale);
    ...
    StatusOr<ColumnPtr> _evaluate_complex(ExprContext* context, const Columns& inputs) { // without only-null columns
        int size = inputs[0]->size();
        Columns columns;
        for (const auto& col : inputs) {
            columns.push_back(ColumnHelper::unfold_const_column(context->get_allocator(), this->type(), size, std::move(col)));
        }
