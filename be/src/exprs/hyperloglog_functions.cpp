...
StatusOr<ColumnPtr> HyperloglogFunctions::hll_cardinality_from_string(FunctionContext* context,
    ...
    return VectorizedStrictUnaryFunction<hllCardinalityFromStringImpl>::evaluate<TYPE_VARCHAR, TYPE_BIGINT>(context->get_allocator(), columns[0]);
}
...
StatusOr<ColumnPtr> HyperloglogFunctions::hll_cardinality(FunctionContext* context, const starrocks::Columns& columns) {
    return VectorizedStrictUnaryFunction<hllCardinalityImpl>::evaluate<TYPE_HLL, TYPE_BIGINT>(context->get_allocator(), columns[0]);
}
...
StatusOr<ColumnPtr> HyperloglogFunctions::hll_hash(FunctionContext* context, const Columns& columns) {
    ...
    auto hll_column = HyperLogLogColumn::create(context->get_allocator());
    ...
    if (ColumnHelper::is_all_const(columns)) {
        return ConstColumn::create(context->get_allocator(), std::move(hll_column), columns[0]->size());
    } else {
        return hll_column;
    }
}
...
StatusOr<ColumnPtr> HyperloglogFunctions::hll_empty(FunctionContext* context, const Columns& columns) {
    auto p = HyperLogLogColumn::create(context->get_allocator());

    p->append_default();
    return ConstColumn::create(context->get_allocator(), std::move(p), 1);
}
...
StatusOr<ColumnPtr> HyperloglogFunctions::hll_serialize(FunctionContext* context, const Columns& columns) {
    return VectorizedStringStrictUnaryFunction<HllSerializeImpl>::evaluate<TYPE_HLL, TYPE_VARCHAR>(context->get_allocator(), columns[0]);
}
