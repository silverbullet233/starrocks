    ...
    static ColumnPtr _array_difference(FunctionContext* ctx, const Columns& columns) {
        ...
        if constexpr (lt_is_decimal<LT>) {
            ...
            dest_column_data = NullableColumn::create(
                    ctx->get_allocator(),
                    RunTimeColumnType<ResultType>::create(ctx->get_allocator(), desc.children[0].precision, desc.children[0].scale),
                    NullColumn::create(ctx->get_allocator()));
        } else {
            dest_column_data = NullableColumn::create(ctx->get_allocator(), RunTimeColumnType<ResultType>::create(ctx->get_allocator()), NullColumn::create(ctx->get_allocator()));
        }
        ...
        if (columns[0]->is_nullable()) {
            ...
            auto dest_column_mut = NullableColumn::create(
                    ctx->get_allocator(),
                    ArrayColumn::create(ctx->get_allocator(), std::move(dest_column_data),
                                       src_data_column->offsets_column()->clone(ctx->get_allocator())),
                    NullColumn::create(ctx->get_allocator()));
            ...
            const auto* src_data_column = down_cast<const ArrayColumn*>(src_column.get());
            auto dest_column_mut = ArrayColumn::create(ctx->get_allocator(), std::move(dest_column_data),
                                                       src_data_column->offsets_column()->clone(ctx->get_allocator()));
...
public:
    ...
    static StatusOr<ColumnPtr> process(FunctionContext* ctx, const Columns& columns) {
        ...
        if (state->left_is_notnull_const && state->right_is_notnull_const) {
            ...
            return _array_overlap_const(ctx, *state, *columns[1]);
        } else if (state->right_is_notnull_const) {
            return _array_overlap_const(ctx, *state, *columns[0]);
        } else {
            return _array_overlap(ctx, columns);
        }
    }

private:
    static ColumnPtr _array_overlap_const(FunctionContext* context, const ArrayOverlapState<HashSet>& state, const Column& column) {
        const size_t num_rows = column.size();
        const auto [is_const, num_packed_rows] = ColumnHelper::num_packed_rows(&column);

        auto result_data_column = BooleanColumn::create(context->get_allocator(), num_packed_rows, 0);
        ...
        if (is_const) {
            ...
            return ConstColumn::create(context->get_allocator(), std::move(result_data_column), num_rows);
        } else if (result_null_column != nullptr) {
            return NullableColumn::create(context->get_allocator(), std::move(result_data_column), std::move(result_null_column));
        } else {
            return result_data_column;
        }
    }

    static ColumnPtr _array_overlap(FunctionContext* context, const Columns& columns) {
...
public:
    ...
    static ColumnPtr process(FunctionContext* ctx, const Columns& columns) {
        ...
        return _array_intersect<phmap::flat_hash_set<CppTypeWithOverlapTimes, CppTypeWithOverlapTimesHash<LT>,
                                                     CppTypeWithOverlapTimesEqual>>(ctx, columns);
    }

private:
    template <typename HashSet>
    static ColumnPtr _array_intersect(FunctionContext* context, const Columns& original_columns) {
        ...
        NullColumn::MutablePtr null_result = NullColumn::create(context->get_allocator());
...
public:
    static ColumnPtr process(FunctionContext* ctx, const Columns& columns) {
        ...
        ColumnPtr src_column = ColumnHelper::unpack_and_duplicate_const_column(chunk_size, columns[0]);
        if (columns.size() <= 2) {
            return _join_column_ignore_null(ctx, src_column, columns[1], chunk_size);
        } else {
            return _join_column_replace_null(ctx, src_column, columns[1], columns[2], chunk_size);
        }
    }

private:
    static ColumnPtr _join_column_replace_null(FunctionContext* context, const ColumnPtr& src_column, const ColumnPtr& sep_column,
                                               const ColumnPtr& null_replace_column, size_t chunk_size) {
        NullableBinaryColumnBuilder res(context->get_allocator());
    ...
    static ColumnPtr _join_column_ignore_null(FunctionContext* context, const ColumnPtr& src_column, const ColumnPtr& sep_column,
                                              size_t chunk_size) {
        NullableBinaryColumnBuilder res(context->get_allocator());
...
private:
    static ColumnPtr _array_match(FunctionContext* context, const Columns& columns) {
        ...
        auto dest_null_column = NullColumn::create(context->get_allocator(), dest_num_rows, 0);
        auto dest_data_column = BooleanColumn::create(context->get_allocator(), dest_num_rows);
        ...
        ColumnPtr dest_column = NullableColumn::create(context->get_allocator(), std::move(dest_data_column), std::move(dest_null_column));
        if (is_const) {
            dest_column = ConstColumn::create(context->get_allocator(), std::move(dest_column), chunk_size);
        }
        return dest_column;
    }
};
    ...
    static ColumnPtr _array_filter(const ColumnPtr& src_column, const ColumnPtr& filter_column) {
        ...
        if (filter_column->only_null()) {
            if (is_src_const) {
                ...
                dest_data_column->append_default();
                return ConstColumn::create(src_column->get_allocator(), std::move(dest_data_column), chunk_size);
            } else {
        ...
        if (is_src_const && is_filter_const) {
            auto const_column = ConstColumn::create(dest_column->get_allocator(), std::move(dest_column), chunk_size);
...
private:
    ...
    static StatusOr<ColumnPtr> process(FunctionContext* context, const Columns& columns) {
        ...
        if (array_column->is_nullable()) {
            ...
            array_null = NullColumn::static_pointer_cast(nullable->null_column()->clone()->as_mutable_ptr());
        } else {
            array_col = down_cast<ArrayColumn*>(array_column->as_mutable_raw_ptr());
            array_null = NullColumn::create(array_column->get_allocator(), array_column->size(), 0);
        }
        ...
        if constexpr (lt_is_decimal<ResultType>) {
            ...
            result = RunTimeColumnType<ResultType>::create(context->get_allocator(), desc.precision, desc.scale);
        } else {
            result = RunTimeColumnType<ResultType>::create(context->get_allocator());
        }
        ...
        return NullableColumn::create(context->get_allocator(), std::move(result), std::move(array_null));
    }
};
...
#define DEFINE_ARRAY_GENERATE_FN(NAME, TIME_UNIT)                                                                  \
    ...
    static StatusOr<ColumnPtr> array_generate_function_##NAME(FunctionContext* ctx, const Columns& columns) {      \
        ...
        CHECK_EQ(offsets->get_data().back(), elements->size());                                                    \
                                                                                                                   \
        auto dst = ArrayColumn::create(ctx->get_allocator(), std::move(array_elements), std::move(array_offsets));                       \
                                                                                                                   \
        if (all_const_cols) {                                                                                      \
    ...
    static StatusOr<ColumnPtr> process(FunctionContext* ctx, const Columns& columns) {
        ...
        if constexpr (Type == TYPE_DATE || Type == TYPE_DATETIME) {
            ...
            if (nulls == nullptr) {
                ...
                return NullableColumn::create(dst->get_allocator(), std::move(dst), std::move(nulls));
            }
        }
    }
};
...
public:
    ...
    static StatusOr<ColumnPtr> process(FunctionContext* context, const Columns& columns) {
        ...
        if (is_nullable_array) {
            result_column = NullableColumn::create(columns[0]->get_allocator(), std::move(result_column), array_null_column->clone());
            result_column->check_or_die();
        }
        if (is_const_array && is_const_target) {
            result_column = ConstColumn::create(columns[0]->get_allocator(), std::move(result_column), array_column->size());
            result_column->check_or_die();
        }
        return result_column;
    }
...
private:
    ...
    static ColumnPtr _process_with_hash_table(ArrayContainsState* state, const ColumnPtr& targets,
        ...
        auto result_column = ReturnType::create(targets->get_allocator());
    ...
    static ColumnPtr _process_generic(const ColumnPtr& arrays, const ColumnPtr& targets,
        ...
        auto result_column = ReturnType::create(arrays->get_allocator());
...
public:
    ...
    static StatusOr<ColumnPtr> process(FunctionContext* context, const Columns& columns) {
        ...
        if (is_const_left && is_const_right) {
            ...
            auto result_column = BooleanColumn::create(context->get_allocator());
            result_column->append(state->contains);
            return ConstColumn::create(context->get_allocator(), std::move(result_column), columns[0]->size());
        }
        ...
        if (is_nullable_left && is_nullable_right) {
            return _process<true, true>(context, state, left_data_column, right_data_column, left_null_data, right_null_data,
                                        is_const_left, is_const_right);
        } else if (is_nullable_left && !is_nullable_right) {
            return _process<true, false>(context, state, left_data_column, right_data_column, left_null_data, right_null_data,
                                         is_const_left, is_const_right);
        } else if (!is_nullable_left && is_nullable_right) {
            return _process<false, true>(context, state, left_data_column, right_data_column, left_null_data, right_null_data,
                                         is_const_left, is_const_right);
        } else {
            return _process<false, false>(context, state, left_data_column, right_data_column, left_null_data, right_null_data,
                                          is_const_left, is_const_right);
        }
    }
...
private:
    ...
    static ColumnPtr _process(FunctionContext* context, const ArrayContainsAllState* state, const ColumnPtr& left_arrays,
        ...
        if constexpr (NullableLeft || NullableRight) {
            result_null_column = NullColumn::create(context->get_allocator());
        ...
        if constexpr (NullableLeft || NullableRight) {
            return NullableColumn::create(result_column->get_allocator(), std::move(result_column), std::move(result_null_column));
        }
        return result_column;
    }
};
