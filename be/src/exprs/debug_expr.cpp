...
StatusOr<ColumnPtr> DebugFunctions::chunk_memusage(ExprContext* context, Chunk* ptr) {
    if (ptr == nullptr) {
        return ColumnHelper::create_const_column<TYPE_BIGINT>(context->get_allocator(), 0, 1);
    }
    size_t mem_usage = ptr->memory_usage();
    size_t num_rows = ptr->num_rows();
    return ColumnHelper::create_const_column<TYPE_BIGINT>(context->get_allocator(), mem_usage, num_rows);
}

StatusOr<ColumnPtr> DebugFunctions::chunk_check_valid(ExprContext* context, Chunk* ptr) {
    // check chunk valid
    if (ptr == nullptr) {
        return ColumnHelper::create_const_column<TYPE_BOOLEAN>(context->get_allocator(), true, 1);
    }
    ptr->check_or_die();
    size_t num_rows = ptr->num_rows();
    ...
    return ColumnHelper::create_const_column<TYPE_BOOLEAN>(context->get_allocator(), true, ptr->num_rows());
}
