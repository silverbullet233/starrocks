...
public:
    ...
    void update_help(FunctionContext* ctx, size_t chunk_size, const Column** columns, const Column** replace_columns,
        ...
        auto fake_null_column = NullColumn::create(ctx->get_allocator(), columns[0]->size(), 0);
        ...
        if (predicate_column->is_nullable()) {
            ...
            if (nullCount == 0) {
                ...
                fake_null_column = NullColumn::create(ctx->get_allocator(), columns[0]->size(), 1);
            } else {
        ...
        if (data_column->is_nullable()) {
            ...
            new_nullable_column =
                    NullableColumn::create(ctx->get_allocator(), original_nullable_column->data_column(),
                                           fake_null_column);
        } else {
            new_nullable_column =
                    NullableColumn::create(ctx->get_allocator(), data_column, fake_null_column);
        }
