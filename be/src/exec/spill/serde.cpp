...
StatusOr<ChunkUniquePtr> ColumnarSerde::deserialize(SerdeContext& ctx, const BlockPtr block) {
    size_t encoded_size;
    RETURN_IF_ERROR(block->read_fully(&encoded_size, sizeof(size_t)));
    ...
    if (_encode_context != nullptr) {
        ...
        for (size_t i = 0; i < columns.size(); i++) {
            ...
            RETURN_IF_ERROR(block->read_fully(&encode_level, sizeof(uint32_t)));
    ...
    RETURN_IF_ERROR(block->read_fully(buf, encoded_size));
    ...
    // @TODO update bytes
    TRACE_SPILL_LOG << "deserialize chunk from block: " << block->debug_string() << ", encoded size: " << encoded_size
