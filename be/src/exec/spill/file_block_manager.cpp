...
public:
    ...
    std::string debug_string() const override {
        return fmt::format("FileBlock:{}[container={}, len={}]", (void*)this, _container->path(),
                           _size);
    }
...
// @TODO rename
FileBlockManager::FileBlockManager(const TUniqueId& query_id, DirManager* dir_mgr) : _query_id(query_id), _dir_mgr(dir_mgr) {
...
StatusOr<BlockPtr> FileBlockManager::acquire_block(const AcquireBlockOptions& opts) {
    ...
    // @TODO test
#endif
    // @TODO since remote storage dir may changed, we cannot maintain it in exec env
    // @TODO we should pre check capacity? otherwise will make an error
    // @TODO or re-pick container?
    // @TODO should consider which task pool should be used for remote storage/local disk io task
...
StatusOr<FileBlockContainerPtr> FileBlockManager::get_or_create_container(Dir* dir, int32_t plan_node_id,
    ...
    std::string container_dir = dir->dir() + "/" + print_id(_query_id);
    RETURN_IF_ERROR(dir->fs()->create_dir_if_missing(container_dir));
    ASSIGN_OR_RETURN(auto block_container, FileBlockContainer::create(dir, _query_id, plan_node_id, plan_node_name, id));
    RETURN_IF_ERROR(block_container->open());
    return block_container;
}
}