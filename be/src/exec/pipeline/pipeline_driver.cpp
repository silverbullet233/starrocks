        ...
        for (size_t i = _first_unfinished; i < num_operators - 1; ++i) {
            {
                ...
                if (curr_op->is_finished()) {
                    ...
                    // auto defer = DeferOp([]{
                    //         LOG(INFO) << "end reserve bytes: " << CurrentThread::current().get_reserved_bytes();
                    //         CurrentThread::current().release_reserved();
                    //         LOG(INFO) << "after release reserve: " << tls_mem_tracker->debug_string();
                    //     });
                    RETURN_IF_ERROR(return_status = _mark_operator_finishing(next_op, runtime_state));
                    // if (runtime_state->enable_spill() && next_op->mem_resource_manager().releaseable()) {
                    //     LOG(INFO) << "after set_finishing: " << tls_mem_tracker->debug_string();
                    // }
                ...
                if (return_status.ok()) {
                    if (maybe_chunk.value() &&
                        ...
                        total_rows_moved += row_num;
                        {
                            ...
                            RELEASE_RESERVED_GUARD();
                            // auto defer = DeferOp([]{
                            //     // LOG(INFO) << "reserve bytes: " << CurrentThread::current().get_reserved_bytes();
                            //     CurrentThread::current().release_reserved();
                            //     // LOG(INFO) << "after release reserve: " << tls_mem_tracker->debug_string();
                            // });
                            return_status = next_op->push_chunk(runtime_state, maybe_chunk.value());
                            // if (runtime_state->enable_spill() && next_op->mem_resource_manager().releaseable()) {
                            //     LOG(INFO) << "after push chunk: " << tls_mem_tracker->debug_string() << ", return_status: " << return_status.to_string();
                            // }
                        }
                ...
                if (curr_op->is_finished()) {
                    ...
                    //     });
                    RETURN_IF_ERROR(return_status = _mark_operator_finishing(next_op, runtime_state));
                    // return_status = _mark_operator_finishing(next_op, runtime_state);
                    // if (!return_status.ok()) {
                    //     LOG(WARNING) << "mark operator finishing error, " << return_status.to_string();
                    // }
                    new_first_unfinished = i + 1;
                    continue;
                }
...
void PipelineDriver::_adjust_memory_usage(RuntimeState* state, MemTracker* tracker, OperatorPtr& op,
    ...
    if (state->enable_spill() && mem_resource_mgr.releaseable() &&
        ...
        // request_reserved = 0;
        // LOG(INFO) << "before reserve, tracker: " << tls_mem_tracker->debug_string();
        // LOG(INFO) << "begin reserve bytes: " << request_reserved;
        ...
        // LOG(INFO) << "after reserve, tracker: " << tls_mem_tracker->debug_string();
    }
}
...
void PipelineDriver::finalize(RuntimeState* runtime_state, DriverState state) {
    ...
    // LOG(INFO) << "after close operator, tracker: " << tls_mem_tracker->debug_string();
    ...
    // LOG(INFO) << "after finalize operator, tracker: " << tls_mem_tracker->debug_string();
