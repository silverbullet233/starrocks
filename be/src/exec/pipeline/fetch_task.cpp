...
Status FetchTask::_submit_remote_task(RuntimeState* state) {
    ...
    closure->addSuccessHandler([this, closure, host = node_info->host, port = node_info->brpc_port](const FetchTaskContextPtr& ctx, const PLookUpResponse& resp) noexcept {
        ...
        if (resp.status().status_code() != TStatusCode::OK) {
            auto msg = fmt::format("fetch request failed, error: {}, host: {}, port: {}", resp.status().DebugString(), host, port);
            LOG(WARNING) << msg;
