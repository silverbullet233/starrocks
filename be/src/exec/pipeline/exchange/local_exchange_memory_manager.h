...
public:
    ...
    void update_max_memory_usage(size_t limit) {
        _max_memory_usage = limit;
        // LOG(INFO) << "update max memory usage to " << limit;
    }

private:
    // size_t _max_memory_usage = 128 * 1024 * 1024 * 1024UL;     // 128GB
    size_t _max_memory_usage_per_driver = 128 * 1024 * 1024UL; // 128MB
    std::atomic<size_t> _max_memory_usage{128UL * 1024 * 1024 * 1024};
