...
Status LookUpProcessor::_collect_input_columns(RuntimeState* state, const ChunkPtr& request_chunk) {
    ...
    for (const auto& slot_id : row_pos_desc->get_fetch_ref_slot_ids()) {
        ...
        auto col = ColumnHelper::create_column(_parent->_allocator, slot_desc->type(), slot_desc->is_nullable());
    ...
    auto row_source_col = ColumnHelper::create_column(_parent->_allocator, slot_desc->type(), false);
