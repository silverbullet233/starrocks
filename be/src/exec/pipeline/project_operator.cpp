    ...
    Columns result_columns(_column_ids.size());
    {
        ...
        for (size_t i = 0; i < _column_ids.size(); ++i) {
            ...
            if (result_columns[i]->only_null()) {
                auto mutable_col = ColumnHelper::create_column(_expr_ctxs[i]->get_allocator(), _expr_ctxs[i]->root()->type(), true);
                ...
                MutableColumnPtr new_column = ColumnHelper::create_column(_expr_ctxs[i]->get_allocator(), _expr_ctxs[i]->root()->type(), false);
            ...
            if (_type_is_nullable[i] && !result_columns[i]->is_nullable()) {
                result_columns[i] =
                        NullableColumn::create(_allocator, std::move(result_columns[i]),
                                               NullColumn::create(_allocator, num_rows, 0));
            }
        }
