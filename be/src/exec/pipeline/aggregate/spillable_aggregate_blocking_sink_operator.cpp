...
Status SpillableAggregateBlockingSinkOperator::set_finishing(RuntimeState* state) {
    ...
    LOG(INFO) << "agg spill set_finishing, " << this;
    ...
    if (!_aggregator->spill_channel()->has_task()) {
        if (_aggregator->hash_map_variant().size() > 0 || !_non_agg_chunks.empty()) {
            LOG(INFO) << "spill res data, agg rows: " << _aggregator->hash_map_variant().size() << ", non agg rows: " << _non_agg_rows;
    ...
    auto set_call_back_function = [this](RuntimeState* state, auto io_executor) {
        return _aggregator->spiller()->set_flush_all_call_back(
                [this, state]() {
                    ...
                    RETURN_IF_ERROR(AggregateBlockingSinkOperator::set_finishing(state));
                    LOG(INFO) << "agg spill done, " << this << ", append rows: " << _aggregator->spiller()->spilled_append_rows();
                    return Status::OK();
                },
...
Status SpillableAggregateBlockingSinkOperator::push_chunk(RuntimeState* state, const ChunkPtr& chunk) {
    ...
    auto preaggregation_mode = state->spill_preaggregation_mode();
    switch (preaggregation_mode) {
        case TSpillPreaggregationMode::AUTO:
            return _try_to_spill_by_auto(state, chunk);
        case TSpillPreaggregationMode::FORCE_SPILL:
...
void SpillableAggregateBlockingSinkOperator::_add_non_agg_chunk(ChunkPtr chunk) {
    _non_agg_rows += chunk->num_rows();
    _non_agg_bytes += chunk->memory_usage();

    _non_agg_chunks.push(std::move(chunk));
}
...
Status SpillableAggregateBlockingSinkOperator::_try_to_spill_by_auto(RuntimeState* state, const ChunkPtr& chunk) {
    ...
    if (!ht_need_expansion ||
        ...
        if (hit_count == 0) {
            ...
            _add_non_agg_chunk(std::move(tmp));
            ...
            }
            {
                ...
                _add_non_agg_chunk(std::move(res));
            }
        }
        if (hit_count * 1.0 / chunk_size <= state->spill_agg_ht_reduction()) {
            _continuous_limit++;
        }
    ...
    const int32_t low_reduction_threshold = state->spill_agg_low_reduction_limit();
    size_t revocable_mem_bytes = _non_agg_bytes + _aggregator->hash_map_memory_usage();
    set_revocable_mem_bytes(revocable_mem_bytes);
    if (revocable_mem_bytes > state->spill_mem_table_size() || _continuous_limit >= low_reduction_threshold) {
        bool should_spill_hash_table = _continuous_limit >= low_reduction_threshold || _aggregator->hash_map_memory_usage() > state->spill_mem_table_size();
...
Status SpillableAggregateBlockingSinkOperatorFactory::prepare(RuntimeState* state) {
    ...
    if (state->spill_preaggregation_mode() == TSpillPreaggregationMode::AUTO) {
