...
template <typename T>
void BinaryColumnBase<T>::fnv_hash_with_selection(uint32_t* hashes, uint8_t* selection, uint16_t from, uint16_t to) const {
    for (uint32_t i = from; i < to; ++i) {
        if (!selection[i]) {
            continue;
        }
...
template <typename T>
void BinaryColumnBase<T>::crc32_hash_with_selection(uint32_t* hashes, uint8_t* selection, uint16_t from, uint16_t to) const {
    for (uint32_t i = from; i < to && !_bytes.empty(); ++i) {
        if (selection[i]) {
            continue;
        }
