...
public:
    ...
    [[nodiscard]] Status do_visit(const NullableColumn& column);
    [[nodiscard]] Status do_visit(const BinaryColumn& column);

    template <typename T>
    [[nodiscard]] Status do_visit(const FixedLengthColumn<T>& column) {
        get_buffer_data(column, &_buffers);
        return Status::OK();
    }

    template <typename T>
    [[nodiscard]] Status do_visit(const T& column) {
