...
public:
    ...
    // get data by rowid
    Status get_next(Chunk* chunk, const rowid_t* rowid, size_t count) {
        Status st = do_get_next(chunk, rowid, count);
        DCHECK_CHUNK(chunk);
        return st;
    }
...
protected:
    ...
    virtual Status do_get_next(Chunk* chunk, const rowid_t* rowid, size_t count) {
        return Status::NotSupported("Chunk* chunk, const rowid_t* rowid, size_t count) not supported");
    }
