...
Status ArrayColumnIterator::next_batch(size_t* n, Column* dst) {
    ...
    // 3. Read elements
    if (_access_values) {
        RETURN_IF_ERROR(_element_iterator->next_batch(&num_to_read, array_column->elements_column_raw_ptr()));
    } else {
...
Status ArrayColumnIterator::fetch_values_by_rowid(const rowid_t* rowids, size_t size, Column* values) {
    ...
    if (_access_values) {
        for (size_t i = 0; i < size; ++i) {
            ...
            RETURN_IF_ERROR(_element_iterator->next_batch(&size_to_read, array_column->elements_column_raw_ptr()));
        }
    } else {
        if (!array_column->elements_column()->is_constant()) {
            array_column->elements_column_raw_ptr()->append_default(1);
        ...
        array_column->elements_column_raw_ptr()->append_default(size_to_read);
    }
...
Status ArrayColumnIterator::fetch_dict_codes_by_rowid(const rowid_t* rowids, size_t size, Column* values) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        RETURN_IF_ERROR(_element_iterator->seek_to_ordinal(element_ordinal));
        size_t size_to_read = array_size.get_data()[i];
        RETURN_IF_ERROR(_element_iterator->next_dict_codes(&size_to_read, array_column->elements_column_raw_ptr()));
    }
