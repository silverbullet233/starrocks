...
Status JsonFlatColumnIterator::next_batch(size_t* n, Column* dst) {
    ...
    if (dst->is_nullable()) {
        ...
        json_column = down_cast<JsonColumn*>(nullable_column->data_column_raw_ptr());
...
Status JsonDynamicFlatIterator::_dynamic_flat(Column* output, FUNC read_fn) {
    ...
    if (output->is_nullable()) {
        ...
        auto* output_null = down_cast<NullColumn*>(output_nullable->null_column_raw_ptr());

        auto* input_nullable = down_cast<NullableColumn*>(proxy.get());
        auto* input_null = down_cast<NullColumn*>(input_nullable->null_column_raw_ptr());
        ...
        // json column
        json_data = down_cast<JsonColumn*>(output_nullable->data_column_raw_ptr());
    } else {
...
public:
    explicit JsonExtractIterator(ColumnIteratorUPtr source_iter, bool source_nullable, JsonPath path, LogicalType type)
            : ColumnIteratorDecorator(source_iter.release(), kTakesOwnership),
              _path(std::move(path)),
              _type(type),
              _source_nullable(source_nullable),
    ...
    Status init(const ColumnIteratorOptions& opts) override {
        RETURN_IF_ERROR(_parent->init(opts));
        // prepare the source chunk
        auto column = ColumnHelper::create_column(opts.allocator, TypeDescriptor::create_json_type(), _source_nullable);
        _source_chunk.append_column(std::move(column), SlotId(0));
        auto path_column = ColumnHelper::create_const_column<TYPE_VARCHAR>(opts.allocator, _path.to_string(), 1);
        ...
        auto const_path = ColumnHelper::create_const_column<TYPE_VARCHAR>(opts.allocator, _path.to_string(), 1);
