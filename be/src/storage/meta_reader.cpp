...
Status MetaReader::_fill_result_chunk(Chunk* chunk) {
    for (size_t i = 0; i < _collect_context.result_slot_ids.size(); i++) {
        ...
        if (field == META_DICT_MERGE) {
            ...
            MutableColumnPtr column = ColumnHelper::create_column(memory::get_default_allocator(), desc, _has_count_agg);
            ...
            desc.children.emplace_back(item_desc);
            MutableColumnPtr column = ColumnHelper::create_column(memory::get_default_allocator(), desc, true);
            chunk->append_column(std::move(column), slot->id());
            ...
            desc.type = TYPE_ARRAY;
            desc.children.emplace_back(item_desc);
            MutableColumnPtr column = ColumnHelper::create_column(memory::get_default_allocator(), desc, false);
            chunk->append_column(std::move(column), slot->id());
