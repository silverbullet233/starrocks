...
auto field_type_dispatch_column(LogicalType ftype, Functor fun, Args&&... args) {
    switch (ftype) {
        ...
        // TYPE_GERMAN_STRING is a query-path rewrite target the scan operator
        // plants onto the storage Schema so ChunkHelper's chunk factory
        // allocates GermanStringColumn instead of BinaryColumn. The dispatch
        // routes through StorageColumnType<TYPE_GERMAN_STRING> (GermanStringColumn).
