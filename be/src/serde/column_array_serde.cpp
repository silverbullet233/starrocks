...
public:
    ...
    Status do_visit(RowIdColumn* column) {
        ...
        _cur = FixedLengthColumnSerde<uint32_t, false>::deserialize(_cur, column->seg_ids_column().get(), _encode_level);
        _cur = FixedLengthColumnSerde<uint32_t, false>::deserialize(_cur, column->ord_ids_column().get(), _encode_level);
        return Status::OK();
    }
